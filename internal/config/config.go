...
func Load() (Config, error) {
	...
	if err != nil || len(c.IdentifierHMACKey) < 32 {
		return Config{}, fmt.Errorf("PARTY_IDENTIFIER_HMAC_KEY must be base64 encoding of at least 32 bytes")
	}
	if c.DatabaseURL == "" {
		return Config{}, fmt.Errorf("PARTY_DATABASE_URL is required")
	}
	if c.AuthDisabled && c.Environment != "local" && c.Environment != "sandbox" {
		return Config{}, fmt.Errorf("authentication can only be disabled locally or in sandbox")
	}
	if !c.AuthDisabled && (c.OIDCIssuer == "" || c.OIDCAudience == "") {
...
func secret(k string) (string, error) {
	...
	if direct != "" && path != "" {
		return "", fmt.Errorf("%s and %s_FILE cannot both be set", k, k)
	}
	if path == "" {
		return direct, nil
	}
